...
jQuery(function($){
  ...
  function page_ajax_load(link_href, bo_gall_id){
    //ajax 로 데이터를 가져오고,
    $.ajax({
      url: link_href
    }).done(function(html) {
        ...
        //데이터 가져온것을 화면을 교체하고,
        var bo_gall_parent = $(html).find(bo_gall_id).parent("div").html();

        $(bo_gall_id).parent("div").html(bo_gall_parent);
        ...
        //스크롤 위치를 옮기고
        var pos_top = $(bo_gall_id).parent("div").position();

          $("html, body").animate({scrollTop:pos_top.top  },"slow");
  ...
  $(document).on("click", ".pg_wrap .pg_page", function(event){
      ...
      var $this = $(this),
          ...
          page_ajax_load(link_href, bo_gall_id);
...
width: 100%;
height: 100%;
background: url(<?php echo $board_skin_url;?>/img/30.gif) center center no-repeat;
}
